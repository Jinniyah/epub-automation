...
### "Welcome back" — full resume (`06-safety-error-handling.md` §Long-run resilience)

| Method & path | Body | Response | Notes |
|---|---|---|---|
| `GET /api/welcome-back` | — | `{"pending_book_ids": [str, ...]}` | Answers "is anything pending" from `state.json` alone. The live `BatchRunner` these ids refer to is reconstructed separately, once, at process startup (`backend/app.py::_build_app_state()` → `_build_runner(restore=True)` → `BatchRunner.restore_books()`, docs/BACKLOG.md Epic 9) — not by this route itself. By the time this route or `/api/status` is ever polled, the runner already knows about every id it lists, so the Epic 8 "Welcome back" screen (`frontend/src/screens/WelcomeBack.tsx`) now shows real titles/phase in both the "tab closed, process still running" case *and* the "genuine backend restart" case; its honest plain-count degradation path only still matters for a book that died before its first status change was ever persisted (or a pre-Epic-9 state file with no snapshots at all). |
| `POST /api/cleanup-in-progress` | — | `{"ok": true}` | "⚙️ More options" → the blunt "clean up stuck in-progress state" escape hatch (docs/BACKLOG.md Epic 9, real user report) for when full resume above can't help (she deleted the source files herself) or isn't wanted. `backend/bridge.py::reset_all_in_progress()` does a best-effort sweep of every `Library/*` stage folder plus a full `state.json` reset, deliberately not correlated with any live tracked `Book`; the route then replaces the in-memory runner (`AppState.new_runner()`) so it can't hold a reference to now-deleted files. Never touches `audit_log.csv`. Frontend confirms before firing, same pattern as Cancel's own confirm dialog. |
