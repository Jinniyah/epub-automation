...
source projects it merges. Checked directly against both the PyPI/
GitHub license listings for the dependencies in question, and against
the three source repos themselves
([`epub-renamer`](https://github.com/Jinniyah/epub-renamer),
[`epub-sanitize`](https://github.com/Jinniyah/epub-sanitize),
[`epub-to-audio`](https://github.com/Jinniyah/epub-to-audio)) — not
assumed from secondhand description — the following holds:

- `epub-renamer` and `epub-to-audio` both carry an MIT `LICENSE.txt` in
  their repo root, confirmed directly.
- `epub-sanitize` has **no license file at all** — just
  `PS_Run-CleanUpEpub.ps1`, `profanity.txt`, and `.gitignore` — consistent
  with it being "the same author's own work" with nothing separate to
  reconcile when folded into this MIT-licensed project.

Separately, this surfaced two bundled *runtime dependencies* that are
...
author's own code (and the MIT-licensed source projects above). This is
a real distinction affecting the distributed artifact, not a technicality
a NOTICE file alone resolves.
...
- [`epub-renamer` repository](https://github.com/Jinniyah/epub-renamer)
  — `LICENSE.txt` (MIT), confirmed directly
...
- [`epub-sanitize` repository](https://github.com/Jinniyah/epub-sanitize)
  — no license file present, confirmed directly
- `docs/requirements/10-licensing-and-notices.md` (full)
- `docs/requirements/00-overview-and-goals.md` §Secondary goal:
  portfolio piece
- `docs/requirements/01-architecture.md` (project structure — inline
  license notes on `mutagen`/`ebooklib` usage)
