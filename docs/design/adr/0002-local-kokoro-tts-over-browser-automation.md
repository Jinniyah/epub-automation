...
directly sourced from `epub-to-audio/epub_utils.py` and `epub2audio.py`.
...
in `docs/requirements/06-safety-error-handling.md` computable at all.
...
- Removes a dependency on Perchance's server-side load/GPU availability
  ...
  (`epub-to-audio`'s own README notes budgeting "2–5 hours for a full
  novel" due to shared-service GPU contention — this is the exact
...
- Introduces a new open verification item, not yet closed: local Kokoro
  ...
  concrete pre-launch QA checklist
  (`docs/requirements/08-open-questions-and-assumptions.md` item 2), not
  a blocking design question — but real verification is still
  outstanding.
- `MAX_CHUNK_CHARS = 4,000`, inherited verbatim from the Perchance-era
  code (confirmed as `epub-to-audio`'s own `--max-chunk` default), was
  tuned for what Perchance's API would accept per request — not for
  Kokoro. Carried over unchanged for now as the right default (reuse
  working logic), but flagged for re-validation once real Kokoro samples
  exist.
- CPU-only inference speed on actual target hardware is unverified;
  ...
  `docs/requirements/03-gui-ux-design.md` §Screen: Working).
...
- [`epub-to-audio` repository](https://github.com/Jinniyah/epub-to-audio)
  — see `epub2audio.py` (Selenium/Perchance driver being replaced),
  `epub_utils.py` (`chunk_text()`, `MAX_CHUNK_CHARS`/`--max-chunk`
  default of 4000), `retag.py`
- `docs/requirements/04-tts-engine.md` (full)
