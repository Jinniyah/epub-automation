...
- React over server-rendered Flask templates: keeps Flask a pure JSON
  ...
  `docs/requirements/01-architecture.md`), and gives a more
  conventional, portfolio-appropriate production architecture.
...
- Closing the browser tab/window is always safe — generation continues
  ...
  (`docs/requirements/03-gui-ux-design.md` §Screen: Working) so she
  doesn't have to intuit it.
- A **new** failure mode is introduced that pywebview wouldn't have:
  ...
  fallback (`docs/requirements/07-packaging-deployment.md`
  §Browser-launch fallback: retry once, then a native `tkinter` dialog
  with the address pre-copied to clipboard).
...
## References
- `docs/requirements/01-architecture.md` §Tech stack summary, §Why these
