...
2. **Progress-reporting simplicity and honesty** — the GUI's polling-
   based status contract (`docs/requirements/01-architecture.md` §Status
...
because the GUI doesn't need it. Notably, the original
[`epub-to-audio`](https://github.com/Jinniyah/epub-to-audio) tool never
had a parallelism concept at all — it's a single-book CLI tool
(`epub2audio.py book.epub [options]`); serial batch processing across
multiple books is itself new territory this project introduces, not
something being preserved from the source.
...
- For large batches (e.g. 15 books) via the GUI, total wall-clock time
  ...
  review (`docs/requirements/08-open-questions-and-assumptions.md`).
...
- [`epub-to-audio` repository](https://github.com/Jinniyah/epub-to-audio)
  — `epub2audio.py`, single-book CLI, no batch/parallelism concept
  (batch looping and this reserved flag are both new to epub-automation)
- `docs/requirements/01-architecture.md` §CLI: reserved `--workers` flag
- `docs/requirements/02-pipeline-stages.md` §Stage 3 Batch behavior
- `docs/requirements/08-open-questions-and-assumptions.md` (one book at
  a time item)
