...
| `pipeline/audio_stage.py` | Real (Epic 4, extended Epic 6). `AudioStage` — per-book chapter/part TTS loop, ID3 tagging via mutagen, part-level resume, retry-then-error. Reuses `rename_stage.build_filename()` directly (minus `.epub`) for the output folder/file base name. Epic 6 added `on_progress`/`should_stop` Observer-pattern hooks (optional, default `None`, backward-compatible) — `pipeline/batch_runner.py`'s only way to report progress and implement Pause/Cancel without this stage ever knowing an HTTP server or batch runner exists. **Real bug found and fixed 2026-07-18, real user report:** `_generate_with_retry()` discarded the real exception on every attempt, leaving the book's `error` field (the only channel `current_error_detail()`/the support bundle ever reads) with a bare "chapter 1, chunk 1" sentence and nothing underneath it. Now returns `(bytes | None, detail)`; the last attempt's `TypeName: message` gets appended to `error` on final failure. **Restructured 2026-07-20 (ADR-0020, real user listening feedback):** chunks within a chapter now group into deterministic ~15-minute "parts" (`group_chunks_into_parts()`), accumulating raw PCM per chunk (`generate_pcm()`) and encoding+writing exactly one MP3 per part instead of one per ~4,000-char chunk. Two counters (`track_num`/`flushed_chunk_count`) keep `on_progress` firing per-chunk (frontend-unaffected) while `chunks_done` on Pause/Cancel/error only ever counts fully-flushed parts — resume/interruption loss unit changed from "one chunk" to "up to one in-progress part," a deliberate bounded tradeoff. ID3 track numbers now count parts, not chunks. Chunk-suffix filenames also gained zero-padding (`006_1.mp3`→`006_001.mp3`) the same day, fixing a real alphabetical-sort playback-order bug found via that same listening report. | Epic 4/6, 2026-07-20 |
...
| `pipeline/tts_engine.py` | Real (Epic 4, extended Epic 8). `TTSEngine` wraps `kokoro.KPipeline`, lazily imported/constructed (first real call only). `VOICES`/`DEFAULT_VOICE`, `estimate_audio_bytes()` (disk-space formula), `ensure_voice_samples()` (cache + version-tagging, now typed against `TTSEngineLike` rather than the concrete class — one less coupling point). MP3 encoding via `lameenc` at Kokoro's native 24kHz — see session notes below. Epic 8 added `installed_kokoro_version()` (package-metadata-only, never imports `kokoro` itself) to finally wire up `ensure_voice_samples()`'s cache-invalidation trigger, deferred since Epic 4/6. **2026-07-20 (ADR-0020):** `generate()` split into `generate_pcm()` (returns raw float32 PCM, what `AudioStage` now calls per chunk) + `generate()` as a thin wrapper (`encode_mp3(generate_pcm(...))`, unchanged for voice-sample callers); `_encode_mp3` renamed to public `encode_mp3()`; new `TARGET_PART_MINUTES`/`MAX_PART_CHARS` constants. | Epic 4 / Epic 8, 2026-07-20 |
...
| `pipeline/epub_utils.py` | Real (Epic 3, extended Epic 4). `sanitize_filesystem_name()` (ADR-0016) — new shared utility, used by rename, audio (Epic 4), and retag (Epic 5). `extract_chapters()`/`chunk_text()`/`normalise_heading()`/`MAX_CHUNK_CHARS` (Epic 4) ported verbatim from `epub-to-audio\epub_utils.py`. **2026-07-20 (ADR-0019):** `extract_chapters()`'s title detection broadened past "first `<h1>`-`<h3>`" — new `_extract_heading_title()` (joins headings clustered before real narrative text, fixing a real book whose named title lived in a separate `<h4>` after a generic `<h3>Chapter N</h3>`) and `_find_de_facto_title()` (best-effort fallback for headingless documents whose title is a plain styled `<p>`, real example: "Pilot"/"Senator" in The Risen Empire). **2026-07-20 (ADR-0020):** new `group_chunks_into_parts()` — deterministic char-count grouping of already-chunked text into ~15-minute "parts" for `AudioStage` to merge into one file each. | Epic 3/4/5, 2026-07-20 |
...
| `tests/test_rename_stage.py`, `test_ai_providers.py`, `test_openai_provider.py`, `test_gemini_provider.py`, `test_epub_reader.py`, `test_epub_utils.py` | Epic 3 test suite (`test_epub_reader.py`/`test_epub_utils.py` extended Epic 4 — see below), 56 tests in `test_epub_utils.py` alone: `build_filename`/`FILENAME_PATTERN`/`RenameStage` (happy path, already-normalized, dry-run, name-conflict, AI failure fallback, corrupted EPUB), provider base/registry/Null/OpenAI/Gemini, `sanitize_filesystem_name` (incl. idempotency). **2026-07-20 additions (ADR-0019/0020):** heading-join and de-facto-title fallback coverage for `extract_chapters()` (mirroring the real Risen Empire/Dragon Reborn cases directly), and a full `group_chunks_into_parts()` section (grouping, oversized-chunk edge case, determinism). | Epic 3, 2026-07-20 |
| `tests/test_retag_stage.py` | Epic 5 test suite, 30 tests. Folder-name parsing (old + new shapes), chapter-title/track-number derivation from MP3 filename suffix, ID3 tag rewriting, override-vs-parsed precedence, dry-run, idempotency, and the folder-rename regression test (`test_run_renames_folder_not_just_files`). **2026-07-20 addition:** a documentation/regression test confirming the filename-suffix parsing needed zero code change once the suffix started meaning a merged "part" (ADR-0020) rather than a raw chunk. | Epic 5, 2026-07-20 |
| `tests/test_tts_engine.py`, `test_audio_stage.py` | Epic 4 test suite, extended 2026-07-20 (ADR-0020). Fake `pipeline_factory`/fake TTS engine throughout — never downloads or runs the real Kokoro model. Covers: MP3 bitrate/sample-rate/mono verification, per-lang-code pipeline caching, `estimate_audio_bytes()` formula, `ensure_voice_samples()` version-mismatch/offline-failure behavior; part/chapter naming conventions, part-level resume, retry-then-error, ID3 tagging incl. cover art, unknown-voice/missing-file/corrupted-EPUB/no-chapters error paths. **2026-07-20 additions:** `test_tts_engine.py` — `generate_pcm()` coverage, a direct proof `generate()` is a thin wrapper around `generate_pcm()`+`encode_mp3()`, `MAX_PART_CHARS` formula guard. `test_audio_stage.py` — the fake TTS engine now returns genuine valid PCM (runs through the real `encode_mp3()`); several tests inverted to prove the new "nothing flushes until the whole part succeeds" resume/Pause/error semantics (a real, deliberate, bounded regression from per-chunk-file resume); new tests for merging reducing file count, ID3 track numbers counting parts not chunks, an oversized single chunk still forming one valid part, and full-part-skip-without-any-TTS-call on resume. `test_epub_utils.py`/`test_epub_reader.py` extended with `extract_chapters()`/`chunk_text()`/`normalise_heading()`/`extract_cover_bytes()` coverage. | Epic 4, 2026-07-20 |
...
**2026-07-20, real user listening feedback: "the audio files cut off and
start in strange locations. Like the middle of a sentence."** Real
report from actually listening to a generated audiobook ("The Risen
Empire"). Investigation (real files inspected directly on disk, plus the
real source EPUBs' internal XHTML/CSS structure) found three distinct,
real problems, not one:

1. **Chunk filename sort order (mechanical bug, fixed first):**
   `pipeline/audio_stage.py` wrote unpadded chunk suffixes
   (`006_1.mp3`...`006_10.mp3`...`006_2.mp3`), which sort alphabetically
   as 1, 10, 11, ..., 2, 20, ... on any player/device that orders files
   by name rather than ID3 track number (most basic phone players, car
   stereos) -- scrambling playback for any chapter with 10+ chunks. A
   real chapter in the reported book had 53. Fixed by zero-padding to 3
   digits, matching the existing chapter-index width. The real "Risen
   Empire" files on disk (153 of them) were renamed in place to match,
   verified correct sort order after.
2. **Chapter-title detection (ADR-0019):** direct inspection of the real
   EPUB's XHTML showed `extract_chapters()`'s "first `<h1>`-`<h3>`"
   heuristic silently failed for this specific book -- chapter names
   ("Pilot", "Senator") are bold-styled `<p>` tags via a CSS class, never
   a real heading tag, so the title stayed empty and ID3 tags fell back
   to a generic "Chapter N". A second real book in the same library
   ("The Dragon Reborn," Wheel of Time #3) showed a *different* failure
   mode when checked at the user's own prompt: it has real headings, but
   two of them (`<h3>Chapter 1</h3>` then a separately-tagged
   `<h4>Waiting</h4>`), and the original heuristic only ever looked at
   the first one, dropping the real named title. Fixed via broadened
   multi-tag heading collection (joins headings clustered before real
   narrative text starts) plus a best-effort de-facto-title fallback for
   headingless documents -- deliberately bounded, not a claim of
   universal detection, per the user's own direct framing ("not all
   books are the same," volunteered while walking through both real
   examples).
3. **Merged ~15-minute "parts" instead of one file per ~4,000-char chunk
   (ADR-0020):** even with chunks correctly ordered, the audible gap
   between one small file ending and the next beginning was a real,
   separate complaint. Confirmed via the same real EPUB inspection that
   chapter *boundaries* were never the bug -- "Pilot" genuinely is one
   continuous 131,633-character chapter in the source file, not several
   chapters wrongly merged; the chapter is just long. The user chose a
   ~15-minute/~15MB target directly, checked against real per-chapter
   numbers computed from the actual book on disk (chapter 12: 53 chunks
   / 205 min / 188MB unmerged -- confirmed too large for one file on a
   phone/tablet). Implementation: `pipeline/epub_utils.py::group_chunks_into_parts()`
   deterministically groups already-chunked text by character count
   (pure text math, computed before any TTS work, so identical on every
   resumed run -- deliberately not based on actual generated audio
   duration, which risks a different boundary on resume if Kokoro's
   output length isn't bit-for-bit deterministic); `pipeline/tts_engine.py`
   gained `generate_pcm()` (raw PCM, `generate()` is now a thin wrapper
   over it + the newly-public `encode_mp3()`) so `AudioStage` can
   accumulate a whole part's audio and encode it through one `lameenc`
   session, avoiding the splice-point artifacts that concatenating
   already-encoded MP3 byte streams would risk. **Real, deliberate,
   bounded tradeoff:** the resume/Pause/Cancel loss unit changed from
   "one chunk" to "up to one in-progress, not-yet-flushed part" (~15 min
   worst case, never a whole chapter/book) -- `06-safety-error-handling.md`
   updated to state this explicitly rather than silently diverge from
   the code; `AudioStage.run()` tracks two counters
   (`track_num`/`flushed_chunk_count`) specifically to keep `on_progress`
   (unaffected, still fires per chunk, zero frontend change) and
   `chunks_done` (now honestly part-level) from being conflated. ID3
   track numbers now count parts, not chunks.

A Plan-agent design-validation pass (used deliberately given the size of
this change: a new TTS engine method, a changed resume-safety guarantee,
and ripple across every test file's fake TTS engine) caught two real
gaps in the first design draft before any code was written: `on_progress`
needed to fire progressively per chunk, not batched at part-flush
(would otherwise freeze the Working screen's progress bar for up to 15
minutes at a stretch), and the naive `chunks_done = track_num - 1` used
for the old per-chunk unit would have silently lied about what's
actually safe on disk under the new part-based unit. A real
implementation bug was also caught and fixed the same way item 1's real
EPUB testing works throughout this session: an early draft of
`_extract_heading_title()` passed its own unit tests but returned empty
titles against the real Dragon Reborn file -- traced to `soup.find_all(True)`
including container tags (`<html>`/`<body>`) whose `get_text()`
aggregates their entire subtree, blowing the heading-search text budget
on the very first tag before ever reaching a real heading; fixed by
walking `.descendants` (individual `Tag`/`NavigableString` leaves) instead.

504 backend tests / 96.5% coverage, clean `black`/`ruff`/`mypy --strict`.
Two new ADRs: `docs/design/adr/0019-chapter-title-detection-broadened.md`,
`docs/design/adr/0020-merge-audio-chunks-into-per-chapter-parts.md`.
**Live-verified against real Kokoro (not just the fake TTS engine unit
tests use), the model already cached locally:** `TTSEngine.generate_pcm()`
+ `encode_mp3()` produce real, valid 128kbps/24kHz/mono MP3 from real
synthesized speech; a full `AudioStage.run()` pass against a small
synthetic book (real Kokoro, `max_part_chars` set low to force 2 parts)
produced exactly 2 correctly-zero-padded, correctly-merged MP3s with ID3
`TRCK` reading `1/2`/`2/2` (parts, not the underlying chunk count) and
`TIT2` correctly carrying the real chapter title.
