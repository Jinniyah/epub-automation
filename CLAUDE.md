...
| Output collision | Audiobook artifact: distinct "replace" vs. "keep both" prompt (`needs_input.type: "output_collision"` — new type, not in the original four-type list). EPUB artifact: auto-deduped via `_dedupe_path()` instead, **a deliberate scope decision made 2026-07-20** while fixing the EPUB-never-reached-`output_folder` bug — a real `needs_input` prompt for this artifact also needs `backend/bridge.py::derive_batch_state()`'s precedence rule to change (see that function's docstring), which is separately-scoped work; flagged in `docs/BACKLOG.md` Epic 9 for a follow-up. | `06-safety-error-handling.md`, `pipeline/batch_runner.py` |
