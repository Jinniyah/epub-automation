...
> 472 backend tests (~96% coverage), 235 frontend tests across 32 files,
...
| Testing | Backend: `pytest` + `pytest-cov` (80%+ floor), `black`, `ruff`, `mypy --strict`, 472 tests passing. Frontend: Vitest + React Testing Library + `vitest-axe` (80%+ floor), `eslint` incl. `eslint-plugin-jsx-a11y`, `tsc`, 235 tests across 32 files passing (confirmed via a real run 2026-07-18). Both CI-enforced. | Built |
