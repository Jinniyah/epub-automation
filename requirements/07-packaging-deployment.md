...
still trigger the warning for a long while even signed. An EV
certificate does get instant trust, but costs several hundred
dollars/year and requires business verification — disproportionate for
this project's scale, and explicitly not worth it just to remove one
click-through.
...
- **Voice sample pre-generation** — all ~28 voice samples generated once
  and cached, at the same setup moment.

Suggested framing for both: a single *"Setting up for the first time...
this only happens once"* screen, rather than two separate unexplained
delays.
