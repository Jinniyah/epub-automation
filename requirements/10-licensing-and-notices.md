...
projects already merged into it (`epub-renamer` and `epub-to-audio` are
both already MIT; `epub-sanitize` has no separate license file and is
the same author's own work, so folding it into an MIT project is
unproblematic).
